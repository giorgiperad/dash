# 🚀 COPY & PASTE - Vercel Environment Variables

## Direct Link:
**https://vercel.com/giorgiperads-projects/ccx8/settings/environment-variables**

---

## Variable 1: FIREBASE_PROJECT_ID

**Key:**
```
FIREBASE_PROJECT_ID
```

**Value:**
```
ccx-dashboard
```

**Environments:** ✅ Production ✅ Preview ✅ Development

---

## Variable 2: FIREBASE_CLIENT_EMAIL

**Key:**
```
FIREBASE_CLIENT_EMAIL
```

**Value:**
```
firebase-adminsdk-fbsvc@ccx-dashboard.iam.gserviceaccount.com
```

**Environments:** ✅ Production ✅ Preview ✅ Development

---

## Variable 3: FIREBASE_PRIVATE_KEY

**Key:**
```
FIREBASE_PRIVATE_KEY
```

**Value:** (Copy the entire block below)
```
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
```

**Environments:** ✅ Production ✅ Preview ✅ Development

**Note:** You can paste this with actual newlines (each line separate) OR as one continuous line - Vercel handles both.

---

## Variable 4: FIREBASE_DATABASE_URL

**Key:**
```
FIREBASE_DATABASE_URL
```

**Value:**
```
https://ccx-dashboard-default-rtdb.firebaseio.com/
```

**Environments:** ✅ Production ✅ Preview ✅ Development

---

## ✅ After Setting All Variables:

1. Go to **Deployments** tab
2. Click **⋯** (three dots) on the latest deployment
3. Click **"Redeploy"**
4. Wait for deployment to complete (~1-2 minutes)
5. Test: **https://ccx8.vercel.app/**

---

## 🧪 Test Endpoint:

After redeploying, test if variables are set:
- **https://ccx8.vercel.app/api/test**

This will show you which variables are set and which are missing.

